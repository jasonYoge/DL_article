...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            train_bottlenecks, train_ground_truth = get_random_chached_bottlenecks(sess, n_classes, image_lists, BATCH,
                                                                                   'training', jpeg_data_tensor,
                                                                                   bottleneck_tensor)
            loss = sess.run(train_step, feed_dict={ bottleneck_input: train_bottlenecks,
                                             ground_truth_input: train_ground_truth })

            if i % 100 == 0 or i + 1 == STEPS:
                validation_bottleneck, validation_ground_truth = \
                    get_random_chached_bottlenecks(sess, n_classes, image_lists, BATCH,
                ...
                print 'Step %d: Validation accuracy on random sampled ' \
                      '%d exmaples = %.1f%%' % (i, BATCH, validation_accuracy * 100)
